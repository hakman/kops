...
* `spec.rootVolume.encryptionKey` requires the KMS key policy to allow the Karpenter controller role, either by naming it or by delegating to IAM. Instances launched with a key the role cannot use terminate immediately after launch. Spot capacity additionally requires the `AWSServiceRoleForEC2Spot` service-linked role in the key policy. kOps grants the controller role the necessary KMS actions, but it does not manage the key policy.
* The root volume `deviceName` is taken from the image kOps resolves at `kops update cluster` time. Karpenter re-resolves `spec.amiSelectorTerms` when it launches, so a selector that matches images with different root device names (a wildcard name, an SSM parameter that moves, or several architectures) can produce a mapping that adds a volume instead of resizing the root one. Pin the image to an AMI ID, or to an exact name, if the InstanceGroup images do not share a root device name.
