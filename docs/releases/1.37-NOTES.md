...
* The `EC2NodeClass` generated for Karpenter-managed InstanceGroups now configures the root volume from `spec.rootVolume`, which it previously ignored. Nodes that used to boot with the root volume of the AMI now use the same size, type, IOPS, throughput and encryption as ASG-managed nodes, 128 GiB encrypted gp3 by default. This changes the `EC2NodeClass` hash, so Karpenter drifts and replaces its nodes on the first `kops update cluster` after upgrading. InstanceGroups that set `spec.rootVolume.encryptionKey` also need the KMS key policy to allow the Karpenter controller role, see the [Karpenter documentation](https://kops.sigs.k8s.io/operations/karpenter/).
