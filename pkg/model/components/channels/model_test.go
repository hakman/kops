...
func Test_RunChannelsBuilder(t *testing.T) {
	// Disable digest pinning so the golden image strings stay stable.
	featureflag.ParseFlags("-ImageDigest")
	// channelList builds VFS paths from configStore.base, which is memfs:// in
	// these fixtures, so the in-memory backend needs to exist.
