...
const (
	// ManifestLocation is the state-store path where the kops-channels static pod manifest is stored.
	ManifestLocation = "manifests/channels/kops-channels.yaml"

	// channelsInterval is how often the kops-channels pod reconciles addons.
	channelsInterval = 60 * time.Second

	// kubeconfigPath is the host path of the kubeconfig nodeup writes for the
	// kops-channels user; mounted read-only into the container.
	kubeconfigPath = "/var/lib/kops/kubeconfig"
)

// ChannelsBuilder builds the kops-channels static pod manifest at cloudup
// and writes it to the state store. Nodeup copies the manifest verbatim onto
// each control-plane node, mirroring how etcd-manager manifests are delivered.
...
// Build renders the kops-channels pod and registers it as a ManagedFile.
func (b *ChannelsBuilder) Build(c *fi.CloudupModelBuilderContext) error {
	...
	c.AddTask(&fitasks.ManagedFile{
		...
		Location:  fi.PtrTo(ManifestLocation),
		Name:      fi.PtrTo("manifests-channels-kops-channels"),
	})
	return nil
}

// channelList builds the same channel URLs nodeUpConfigBuilder stores in
// NodeupConfig.Channels — toolbox_enroll relies on the bootstrap URL being
...
func (b *ChannelsBuilder) buildPod(channels []string) (*v1.Pod, error) {
	...
	kubemanifest.AddHostPathMapping(pod, &container, "kubeconfig", kubeconfigPath,
	...
	for i, dir := range fileChannelDirs(channels) {
