...
type acrKeychain struct {
	...
	// mu guards auths; Resolve is called for every registry operation, so the
	// token exchange is done once per registry.
	mu    sync.Mutex
...
func (k *acrKeychain) Resolve(target authn.Resource) (authn.Authenticator, error) {
	...
	refreshToken, err := fi.ACRExchangeToken(ctx, registry, token.Token)
	if err != nil {
	...
	auth := authn.FromConfig(authn.AuthConfig{
		Username: fi.ACRTokenUser,
		Password: refreshToken,
	})
	k.auths[registry] = auth
	return auth, nil
}
