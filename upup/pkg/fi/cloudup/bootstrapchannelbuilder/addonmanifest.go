...
type AddonManifest struct {
	...
	source        fi.Resource
	// addonSpec is mutated during Normalize (Version, ManifestHash, Prune) and
	// subsequently read by BootstrapChannel, which depends on this task so the
	// mutations are fully visible by the time it runs.
...
func (a *AddonManifest) Normalize(c *fi.CloudupContext) error {
	...
	// Older versions of channels that may be running on the upgrading cluster requires Version to
	// be set. We hardcode version to a high version to ensure an update is triggered on first run,
	// and from then on only a hash change will trigger an addon update.
	a.addonSpec.Version = "9.99.0"
