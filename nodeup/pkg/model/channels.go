...
// ChannelsBuilder writes the host-side artifacts the kops-channels static pod
// needs: the kops-channels user, the kubeconfig owned by that user, and the
// pod manifest copied from the state store (built by the cloudup channels
// builder).
...
// readChannelsManifest fetches the manifest produced by the cloudup builder
// and applies node-local SELinux decoration when needed. The pod is otherwise
...
func (b *ChannelsBuilder) readChannelsManifest(c *fi.NodeupModelBuilderContext) ([]byte, error) {
	...
	// SELinux setting is per-IG, so it can only be applied at nodeup. Skip the
	// parse/reserialize round-trip when there's nothing to add.
