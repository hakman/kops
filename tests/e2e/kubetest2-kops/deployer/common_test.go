...
func TestCanonicalizeBaseURL(t *testing.T) {
	cases := []struct {
		...
		{
			name:          "aws with GCS staged artifacts is left alone",
			cloudProvider: "aws",
			baseURL:       "https://storage.googleapis.com/k8s-staging-kops/kops/1.34.0/",
			expected:      "https://storage.googleapis.com/k8s-staging-kops/kops/1.34.0/",
		},
		{
			name:          "aws with regional S3 staged artifacts",
			cloudProvider: "aws",
			baseURL:       "https://example-bucket.s3.us-east-2.amazonaws.com/kops/1.34.0/",
			expected:      "s3://example-bucket/kops/1.34.0/",
		},
		{
			name:          "aws with global S3 staged artifacts",
			cloudProvider: "aws",
			baseURL:       "https://example-bucket.s3.amazonaws.com/kops/1.34.0/",
			expected:      "s3://example-bucket/kops/1.34.0/",
		},
		{
